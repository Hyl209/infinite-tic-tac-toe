...
**Goal:** 为注册用户原子分配永久唯一的 6 位玩家 UID，并提供按 UID 或完整用户名精确搜索、好友申请、在线状态和等待房间内邀请好友加入对局的完整闭环。
...
- `profiles.player_uid` 为唯一、不可修改的整数，范围固定为 `0..999999`；所有界面统一补零显示为 6 位。
- 迁移现有账号时，当前管理员优先，其余账号按 `created_at, id` 升序排列；管理员得到 `000000`，第二个账号得到 `000001`，后续依次递增。
- 新注册账号由数据库序列原子分配 UID；客户端不能指定、分配或修改 UID。
- 好友搜索只保留两种精确方式：6 位 ASCII 数字 UID，或规范化后的完整用户名；不提供模糊搜索和全站用户目录。
- 恰好 6 位纯数字的用户名保留给玩家 UID，注册和修改用户名时均禁止使用。
- 玩家中心、好友列表和个人资料中的 UID 均显示补零后的 6 位值。
...
- [x] **Step 3: 实现精确搜索和好友列表 RPC**

`search_player_by_username(p_username text)` 必须使用现有用户名规范化规则，只在完整用户名完全匹配时返回结果；`search_player_by_uid(p_player_uid integer)` 只按范围内 UID 完全匹配。两者返回 `user_id, player_uid, username, game_name, relationship_state`，其中 `player_uid` 补零为 6 位；不得返回邮箱或支持 `%` 模糊匹配。
...
客户端只接受 6 位 ASCII 数字 UID 或规范化完整用户名，并分别路由到对应精确搜索 RPC；所有写操作调用 RPC。`subscribe()` 订阅当前用户可见的 `friend_requests` 和 `game_invites` 变更，收到事件后通知页面重新拉取，不能把 Realtime payload 当最终业务状态。
...
- [x] **Step 1: 新增 friends tab 和稳定 DOM ID**
...
- [x] **Step 2: 实现精确搜索和申请状态**

输入为 6 位 ASCII 数字时按 UID 搜索，否则沿用用户名规则 `[a-z0-9_]{3,20}`；提交后只显示一个精确结果。结果根据 `none / outgoing / incoming / friends / self` 显示唯一有效操作。

- [x] **Step 3: 实现申请和好友列表操作**
...
- 注册用户获得数据库原子分配、唯一且不可修改的 6 位玩家 UID；旧账号按管理员优先及 `created_at, id` 顺序完成回填。
- 注册用户可按 6 位 UID 或完整用户名建立、拒绝和删除好友关系。
- 只有好友能看到在线/最近在线状态。
...
- 隔离 Supabase 项目 `Hyl209's Project` 已完成真实迁移与旧账号回填验证：当前管理员为 `000000`，第二个账号为 `000001`，其余账号严格按 `created_at, id` 顺序递增。
- 以 12 个并发注册请求验证数据库序列原子分配，UID 无重复且均在 `0..999999`；同时验证客户端不能指定 UID、数据库拒绝修改既有 UID。
- 好友搜索分别通过 6 位 UID 和完整用户名完成精确命中、发送申请、接受申请及好友列表回显，所有 UID 均为补零后的 6 位值。
- 在线状态已验证：持续心跳时在线，停止心跳超过 90 秒后转为离线并保留最后在线时间。
- 游戏邀请已验证 `pending → accepted`；两个独立浏览器会话均实时收到好友/邀请变化，并通过现有房间预览和加入流程进入对局。
- 390px 移动宽度下已验收好友列表、邀请对话框和 toast，无遮挡关键操作的问题。
- 隔离项目验收测试数据已全部清零；`player_uid_seq` 恢复为 `last_value = 0, is_called = false`，UID 分配与不可修改触发器均保持启用。
- 现网项目 `infinite-tic-tac-toe` 仅完成只读检查，未应用迁移、未写入数据、未部署本阶段功能。
- 本地验收基线为 `HEAD d7fa745`：fresh 全量 `node --test` 通过 `365/365`，`git diff --check -- . ':!assets/vendor/**'` 通过。

结论：代码与隔离环境验收已完成；现网部署必须另走批准流程，本记录不代表现网已上线。
