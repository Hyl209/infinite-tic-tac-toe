...
function createFakeSupabase({ user = null, profile = null, rpcResult = null } = {}) {
  const calls = [];
    ...
    const query = {
      ...
      async single() {
        ...
        if (action === 'insert' && payload) {
          currentProfile = { ...payload, player_uid: currentProfile?.player_uid ?? 1 };
        }
        if (action === 'update' && payload) currentProfile = { ...currentProfile, ...payload };
        return { data: currentProfile, error: null };
      },
    };
    return query;
  ...
  const client = {
    ...
    async rpc(name, params) {
      ...
      if (name === 'rename_with_item' && Array.isArray(rpcResult) && rpcResult[0]) {
        currentProfile = {
          ...currentProfile,
          username: rpcResult[0].username,
          game_name: rpcResult[0].game_name,
        };
      }
    ...
    from(table) {
      ...
      return {
        ...
        insert(payload) {
          ...
          return profileQuery('insert', payload);
        },
        update(payload) {
          calls.push(['update', payload]);
          return profileQuery('update', payload);
        },
      };
    },
  };
...
test('账号错误转换为明确中文提示', () => {
  ...
  assert.equal(account.mapAccountError(new Error('PLAYER_UID_EXHAUSTED')), '玩家 UID 已发放完毕，请联系管理员');
  assert.equal(account.mapAccountError(new Error('PLAYER_UID_IMMUTABLE')), '玩家 UID 不可修改');
  assert.equal(account.mapAccountError(new Error('INSUFFICIENT_ITEMS')), '需要一张改名卡');
...
test('初始化会恢复已登录账号和游戏名', async () => {
  const fake = createFakeSupabase({
    ...
    profile: { username: 'player_01', game_name: '立哥', player_uid: 0 },
  });
  const client = account.createAccountClient({
    config: { supabaseUrl: 'https://example.supabase.co', supabaseAnonKey: 'anon-key' },
    loadSupabase: async () => ({ createClient: () => fake.client }),
...
test('注册会升级匿名会话并保存独立游戏名', async () => {
  ...
  assert.deepEqual(fake.calls.find((call) => call[0] === 'insert'), [
    'insert',
    { id: 'guest-user', username: 'player_01', game_name: '立哥' },
  ]);
  assert.equal(fake.calls.some((call) => JSON.stringify(call).includes('player_uid')), false);
  assert.equal(identity.uid, '000001');
  assert.equal(identity.displayName, '立哥');
  assert.equal(identity.username, 'player_01');
});

test('登录使用隐藏邮箱并读取资料', async () => {
  const fake = createFakeSupabase({ profile: { username: 'player_01', game_name: '棋手甲', player_uid: 7 } });
  const client = account.createAccountClient({
    config: { supabaseUrl: 'https://example.supabase.co', supabaseAnonKey: 'anon-key' },
    loadSupabase: async () => ({ createClient: () => fake.client }),
    storage: createStorage(),
  ...
  const fakeClient = {
    ...
    from() {
      return {
        ...
        eq() { return this; },
        async maybeSingle() {
          return { data: { username: 'player_01', game_name: '棋手甲', player_uid: 7 }, error: null };
        },
      };
    },
  };
...
test('修改游戏名通过改名卡 RPC 更新身份和剩余库存，退出后恢复持久匿名名', async () => {
  const storage = createStorage({ 'board-game-guest-name': '匿名玩家·ABCD' });
  const fake = createFakeSupabase({
    user: { id: 'registered-user', email: 'player_01@players.invalid', is_anonymous: false },
    profile: {
      id: 'registered-user', username: 'player_01', game_name: '旧名字', player_uid: 7,
    },
    rpcResult: [{
      username: 'player_01', game_name: '新名字', rename_card_quantity: '2',
    }],
  });
  const client = account.createAccountClient({
    config: { supabaseUrl: 'https://example.supabase.co', supabaseAnonKey: 'anon-key' },
  ...
  assert.equal(updated.renameCardQuantity, 2);
  ...
  const renameCall = fake.calls.find((call) => call[0] === 'rpc');
  assert.equal(renameCall[1], 'rename_with_item');
  assert.equal(renameCall[2].p_game_name, '新名字');
  assert.match(renameCall[2].p_request_id, /^[0-9a-f-]{36}$/i);
  assert.equal(fake.calls.some((call) => call[0] === 'update'), false);
